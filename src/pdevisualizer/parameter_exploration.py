...
class ParameterExplorer:
    ...
    def parameter_sweep(
        self,
        parameter_name: str,
        parameter_values: Union[np.ndarray, List[float]],
        ...
        if len(param_values) == 0:
            ...
            print(
                f"Testing {len(param_values)} values: {param_values[0]:.3f} to {param_values[-1]:.3f}"
            )
        ...
        for i, value in enumerate(param_values):
            ...
            solver_params = {
                k: v for k, v in params.items() if k in ["alpha", "c", "dt", "dx", "dy"]
            }
    ...
    def compare_parameters(
        self, param_configs: List[Dict[str, Any]], labels: Optional[List[str]] = None
    ) -> Dict[str, np.ndarray]:
        """
    ...
    def sensitivity_analysis(
        ...
        if n_samples == 1:
            ...
            param_values = np.linspace(
                base_value - perturbation, base_value + perturbation, n_samples
            )
            ...
            for metric_name, metric_values in sweep_result.metrics.items():
                if len(metric_values) > 1:
                    ...
                    if base_metric != 0 and param_change != 0:
                        relative_sensitivity = (max_change / abs(base_metric)) / (
                            param_change / abs(base_value)
                        )
...
class ParameterVisualizer:
    ...
    def plot_solution_comparison(
        solutions: Dict[str, np.ndarray], figsize: Tuple[int, int] = (15, 5), cmap: str = "viridis"
    ) -> Figure:
        """
        ...
        for i, (label, solution) in enumerate(solutions.items()):
            im = axes_list[i].imshow(solution, cmap=cmap, origin="lower", vmin=vmin, vmax=vmax)
    ...
    def plot_parameter_grid(
        ...
        for i, val1 in enumerate(param1_arr):
            for j, val2 in enumerate(param2_arr):
                print(
                    f"  Computing ({i+1},{j+1}): {param1_name}={val1:.3f}, {param2_name}={val2:.3f}"
                )
                ...
                # Solve
                solver = PDESolver(
                    explorer.equation, grid_shape=explorer.grid_shape, boundary=explorer.boundary
                )
                solver.set_parameters(**solver_params)
                solver.set_initial_conditions(
                    explorer.initial_conditions, explorer.initial_velocity
                )
        ...
        for i, val1 in enumerate(param1_arr):
            for j, val2 in enumerate(param2_arr):
                ...
                im = axes_grid[n2 - 1 - j][i].imshow(
                    solution, cmap=cmap, origin="lower", vmin=vmin, vmax=vmax
                )
    ...
    def plot_sensitivity_analysis(
        sensitivity_result: Dict[str, Any], figsize: Tuple[int, int] = (12, 6)
    ) -> Figure:
        """
        ...
        for bar, sens in zip(bars, sensitivities):
            ...
            ax2.text(
                bar.get_x() + bar.get_width() / 2.0, height, f"{sens:.3f}", ha="center", va="bottom"
            )
