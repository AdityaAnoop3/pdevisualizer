...
def apply_boundary_conditions(u, boundary_spec, dx=1.0, dy=1.0, u_prev=None, c=1.0, dt=0.1):
    """
...
def step_wave_with_boundaries(u, u_prev, c, dt, dx, dy, boundary_spec_type, boundary_value):
    ...
    for i in range(1, nx - 1):
        for j in range(1, ny - 1):
            ...
            u_next[i, j] = 2 * u[i, j] - u_prev[i, j] + c**2 * dt**2 * (d2u_dx2 + d2u_dy2)
...
def solve_wave_with_boundaries(
    u0, boundary_spec, v0=None, c=1.0, dt=0.1, dx=1.0, dy=1.0, steps=100
):
    """
    ...
    for i in range(1, nx - 1):
        for j in range(1, ny - 1):
            ...
            u_next[i, j] = u0[i, j] + dt * v0[i, j] + 0.5 * c**2 * dt**2 * (d2u_dx2 + d2u_dy2)
    ...
    for _ in range(steps - 1):
        u_next = step_wave_with_boundaries(
            u_curr, u_prev, c, dt, dx, dy, boundary_type, boundary_value
        )
        u_prev = u_curr
        u_curr = u_next
