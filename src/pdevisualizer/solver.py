...
class InitialConditions:
    ...
    @staticmethod
    def gaussian_pulse(
        shape: Tuple[int, int], center: Tuple[float, float], sigma: float, amplitude: float = 1.0
    ) -> np.ndarray:
    ...
    @staticmethod
    def circular_wave(
        shape: Tuple[int, int], center: Tuple[float, float], radius: float, amplitude: float = 1.0
    ) -> np.ndarray:
    ...
    @staticmethod
    def sine_wave(
        shape: Tuple[int, int], wavelength: float, amplitude: float = 1.0, direction: str = "x"
    ) -> np.ndarray:
...
class PDESolver:
    ...
    def __init__(
        self,
        equation: Union[str, EquationType],
    ...
    def set_initial_conditions(self, u0: np.ndarray, v0: Optional[np.ndarray] = None):
        ...
        if u0.shape != self.grid_shape:
            raise ValueError(
                f"Initial condition shape {u0.shape} doesn't match " f"grid shape {self.grid_shape}"
            )
        ...
        if self.equation == EquationType.WAVE and v0 is not None:
            if v0.shape != self.grid_shape:
                raise ValueError(
                    f"Initial velocity shape {v0.shape} doesn't match "
    ...
    def solve(self, steps: int = 100) -> np.ndarray:
        ...
        if self.equation == EquationType.HEAT:
            ...
            if self.boundary.type == BoundaryType.DIRICHLET and self.boundary.value == 0.0:
                # Use original solver for default case (better performance)
                return solve_heat(
                    self._initial_conditions, α=alpha, dt=dt, dx=self.dx, dy=self.dy, steps=steps
                )
            else:
    ...
    def animate(
        self, frames: int = 100, interval: int = 50, save_path: Optional[str] = None
    ) -> FuncAnimation:
        """
