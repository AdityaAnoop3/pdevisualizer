...
class EnhancedVisualizer:
    ...
    def plot_contours(
        ...
        if fill_contours:
            ...
            ax.contour(
                solution, levels=levels, colors="black", alpha=0.3, linewidths=0.5, origin="lower"
            )
        else:
    ...
    def plot_solution_evolution(
        ...
        for i, (solution, t) in enumerate(zip(solutions, time_points)):
            ...
            if plot_type == "contour":
                cs = ax.contourf(
                    solution, levels=15, cmap=cmap, vmin=vmin, vmax=vmax, origin="lower"
                )
                ax.contour(
                    solution, levels=15, colors="black", alpha=0.3, linewidths=0.5, origin="lower"
                )
            else:  # heatmap
        ...
        if n_solutions > 0 and cs is not None:
            fig.colorbar(
                cs, ax=axes[:n_solutions], orientation="vertical", fraction=0.046, pad=0.04
            )
            ...
            for j, val2 in enumerate(param2_values):
                ...
                try:
                    ...
                    if metric == "max_value":
                        ...
                        center_i, center_j = (
                            explorer.grid_shape[0] // 2,
                            explorer.grid_shape[1] // 2,
                        )
    ...
    def plot_solution_comparison_enhanced(
        ...
        figsize: Tuple[int, int] = (15, 10),
        cmap: str = "viridis",
        plot_types: List[str] = ["heatmap", "contour"],
    ) -> Figure:
        """
    ...
    @staticmethod
    def plot_parameter_sweep_enhanced(
        ...
        if include_contours and current_row < rows:
            for i, (solution, param_val) in enumerate(zip(sweep_result.solutions, param_values)):
                ...
                cs = ax.contourf(
                    solution, levels=15, cmap="viridis", vmin=vmin, vmax=vmax, origin="lower"
                )
                ax.contour(
                    solution, levels=15, colors="black", alpha=0.3, linewidths=0.5, origin="lower"
                )
    ...
    def plot_wave_comparison(
        ...
        for i, (label, solution) in enumerate(solutions.items()):
            im = axes[i].imshow(solution, cmap=cmap, origin="lower", vmin=vmin, vmax=vmax)
    ...
    def plot_heat_comparison(
        solutions: Dict[str, np.ndarray], figsize: Tuple[int, int] = (15, 5)
    ) -> Figure:
        """
        ...
        im = None  # Initialize im variable
        for i, (label, solution) in enumerate(solutions.items()):
            im = axes[i].imshow(solution, cmap="hot", origin="lower", vmin=vmin, vmax=vmax)
            axes[i].set_title(label)
            axes[i].set_xlabel("x")
            if i == 0:
                axes[i].set_ylabel("y")
