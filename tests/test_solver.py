...
class TestInitialConditions:
    ...
    def test_sine_wave_x(self):
        ...
        assert u0.shape == (20, 20)
        assert np.max(np.abs(u0)) == pytest.approx(
            2.0, abs=0.1
        )  # Relaxed tolerance for discrete sampling
    ...
    def test_sine_wave_diagonal(self):
        u0 = InitialConditions.sine_wave(
            (20, 20), wavelength=10, amplitude=2.0, direction="diagonal"
        )
        assert u0.shape == (20, 20)
        assert np.max(np.abs(u0)) == pytest.approx(
            2.0, abs=0.1
...
class TestIntegrationExamples:
    ...
    def test_complete_heat_workflow(self):
        ...
        u0 = InitialConditions.multiple_sources((20, 20), [(10, 10, 100.0), (5, 15, 50.0)])
        solver.set_initial_conditions(u0)
    ...
    def test_complete_wave_workflow(self):
        # Create solver
        solver = PDESolver(equation="wave", grid_shape=(30, 30), spacing=(1.0, 1.0))
