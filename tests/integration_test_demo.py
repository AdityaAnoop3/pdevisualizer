...
def test_heat_equation_parameter_exploration():
    ...
    sweep_result = explorer.parameter_sweep("alpha", alpha_values, custom_params={"steps": 100})
    ...
    sensitivity_result = explorer.sensitivity_analysis(
        "alpha", base_value=0.25, perturbation_percent=20, n_samples=7
    )
    ...
    fig4 = ParameterVisualizer.plot_parameter_grid(
        explorer, "alpha", np.array([0.1, 0.3]), "dt", np.array([0.05, 0.1]), figsize=(10, 8)
    )
...
def test_wave_equation_parameter_exploration():
    ...
    sweep_result = explorer.parameter_sweep("c", c_values, custom_params={"steps": 150})
